...
export class AuthenticationController {
    ...
    async signup(request: FastifyRequest<{Body: CreateUserDto}>, reply: FastifyReply) {
        ...
        try {
            ...
            check(firstName.length >= Constants.MIN_NAME_LENGTH
                && firstName.length <= Constants.MAX_NAME_LENGTH, ApiMessages.USER.INVALID_FIRSTNAME);
            check(lastName.length >= Constants.MIN_NAME_LENGTH
                && lastName.length <= Constants.MAX_NAME_LENGTH, ApiMessages.USER.INVALID_LASTNAME);
            check(age >= Constants.MIN_AGE_VALUE && age <= Constants.MAX_AGE_VALUE, ApiMessages.USER.INVALID_AGE);
        } catch (error) {
